...
check_load_store_for_partial_vectors (
  loop_vec_info loop_vinfo, tree vectype, slp_tree slp_node,
  vec_load_store_type vls_type, int group_size,
  vect_memory_access_type memory_access_type, gather_scatter_info *gs_info,
  tree scalar_mask)
{
  ...
  if (memory_access_type == VMAT_LOAD_STORE_LANES)
    {
      if (is_load ? !vect_load_lanes_supported (vectype, group_size, true)
  ...
  if (memory_access_type == VMAT_GATHER_SCATTER)
    {
	 ...
	 2.For mask generated by compare operation, we use mask_gather_scatter.
      ...
      bool len_p = false;
      internal_fn ifn;
      if (get_len_gather_scatter_mode (vecmode,
				       TYPE_MODE (gs_info->offset_vectype),
				       is_load)
	    .exists (&vmode))
	len_p = true;
	  ...
	  gcc_assert (TREE_CODE_LENGTH (code1) == unary_op);
          ...
          vec_loop_lens *lens =
              (loop_vinfo && LOOP_VINFO_FULLY_WITH_LENGTH_P (loop_vinfo)
                   ...
                   : NULL);
          internal_fn len_fn = get_length_conversion_internal_fn (
  ...
  FOR_EACH_VEC_ELT (vec_oprnds0, i, vop0)
	...
	vop1 = vec_oprnds1[i];
      ...
      bool with_len_loop_p =
          loop_vinfo && LOOP_VINFO_FULLY_WITH_LENGTH_P (loop_vinfo);
      internal_fn len_fn = get_with_length_shift_internal_fn (
          code, VECTOR_MODE_P (TYPE_MODE (TREE_TYPE (vop1))));
			...
			stmt_vector_for_cost *cost_vec)
{
  ...
  if (!vec_stmt) /* transformation not required.  */
	  ...
	  && reduc_idx >= 0)
	{
      ...
      {
        if (targetm.vectorize.loop_len_override_mask ())
          vect_record_loop_len (loop_vinfo, lens, ncopies * vec_num, vectype, 1);
        else
  ...
  */
  ...
  for (j = 0; j < ncopies; j++)
      ...
      if (memory_access_type == VMAT_LOAD_STORE_LANES)
	{
	  ...
	  if (loop_masks)
	    final_mask
	      = vect_get_loop_mask (gsi, loop_masks, ncopies, vectype, j);
	  if (vec_mask)
                    ...
                    if (final_mask)
                      {
                        /* In RISC-V 'V' Extension, if we can brings the
                           length into maskload, we can save length
                           configuration instructions and register that hold
                           length value. */
                        tree ptr =
                        ...
                        if (loop_lens && memory_access_type != VMAT_INVARIANT &&
                          ...
                          call = gimple_build_call_internal (
                              IFN_MASK_LOAD, 3, dataref_ptr, ptr, final_mask);
                        gimple_call_set_nothrow (call, true);
                        new_stmt = call;
                        data_ref = NULL_TREE;
                      }
          ...
          if (bitop1 == NOP_EXPR)
              ...
              if (len &&
                                         ...
                                         unsigned_p) != CODE_FOR_nothing)
                {
                  ...
                  gimple *call = gimple_build_call_internal (
                      unsigned_p ? IFN_LEN_VEC_CMPU : IFN_LEN_VEC_CMP, 4,
                      vec_cond_lhs, vec_cond_rhs, len, code_tree);
          ...
          else
            {
              ...
              if (bitop1 == BIT_NOT_EXPR)
                {
                  if (len && direct_internal_fn_supported_p (
                                 IFN_LEN_NOT, vec_cmp_type, OPTIMIZE_FOR_SPEED))
                    {
                      new_stmt = gimple_build_call_internal (IFN_LEN_NOT, 2,
                                                             vec_cond_rhs, len);
                      gimple_call_set_lhs (new_stmt, new_temp);
                    }
                  else
                    new_stmt =
                        gimple_build_assign (new_temp, bitop1, vec_cond_rhs);
                }
              else
                {
                  internal_fn len_fn = get_with_length_internal_fn (bitop1);
                  if (len && len_fn != IFN_LAST &&
                      direct_internal_fn_supported_p (len_fn, vec_cmp_type,
                                                      OPTIMIZE_FOR_SPEED))
                    {
                      new_stmt = gimple_build_call_internal (
                          len_fn, 3, vec_cond_lhs, vec_cond_rhs, len);
                      gimple_call_set_lhs (new_stmt, new_temp);
                    }
                  else
    ...
    if (bitop1 == NOP_EXPR)
      {
        ...
        if (len &&
                                   ...
                                   unsignedp) != CODE_FOR_nothing)
          {
            ...
            new_stmt = gimple_build_call_internal (
                unsignedp ? IFN_LEN_VEC_CMPU : IFN_LEN_VEC_CMP, 4, vec_rhs1,
                vec_rhs2, len, code_tree);
            gimple_call_set_lhs (new_stmt, new_temp);
          }
        else
        ...
        if (bitop1 == BIT_NOT_EXPR)
          {
            if (len && direct_internal_fn_supported_p (IFN_LEN_NOT,
              ...
              {
                new_stmt =
                    gimple_build_call_internal (IFN_LEN_NOT, 2, vec_rhs2, len);
                gimple_call_set_lhs (new_stmt, new_temp);
              }
            else
        ...
        else
          {
            ...
            if (len_fn != IFN_LAST &&
                direct_internal_fn_supported_p (len_fn, TREE_TYPE (new_temp),
                                                OPTIMIZE_FOR_SPEED))
              {
                new_stmt = gimple_build_call_internal (len_fn, 3, vec_rhs1,
                                                       vec_rhs2, len);
                gimple_call_set_lhs (new_stmt, new_temp);
              }
            else
              new_stmt =
                  gimple_build_assign (new_temp, bitop1, vec_rhs1, vec_rhs2);
          }
        vect_finish_stmt_generation (vinfo, stmt_info, new_stmt, gsi);
        if (bitop2 != NOP_EXPR)
          {
            ...
            if (bitop2 == BIT_NOT_EXPR)
            ...
            else
              {
                ...
                if (len_fn != IFN_LAST &&
                  ...
                  new_stmt =
                      gimple_build_assign (res, bitop2, vec_rhs1, new_temp);
              }
            vect_finish_stmt_generation (vinfo, stmt_info, new_stmt, gsi);
          }
      }
...
void
vect_add_len_without_overflow (gimple_seq *header_seq, tree base,
                               class loop *loop, tree ctrl, tree compare_type,
                               tree ctrl_type, tree base_factor,
                               ...
                               tree *var_before, tree *var_after)
{
  ...
  *var_after = va;
  ...
  tree var = gimple_build (
      header_seq, IFN_WHILE_LEN, compare_type, ctrl,
  ...
  if (after)
    {
      if (gsi_end_p (*incr_pos) ||
          (is_gimple_debug (gsi_stmt (*incr_pos)) && gsi_bb (*incr_pos) &&
