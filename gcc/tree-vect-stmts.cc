				      ...
				      tree scalar_mask)
{
  ...
  if (memory_access_type == VMAT_GATHER_SCATTER)
    {
      /* For RISC-V 'V' Extension, we need len_gather_scatter and
         mask_gather_scatter both.
	 1.For Loop counter control we use len_gather_scatter, higher priority.
	 2.For mask generated by compare operation, we use mask_gather_scatter. */
      machine_mode vmode;
      bool len_p = false;
      internal_fn ifn;
      if (get_len_gather_scatter_mode (vecmode, TYPE_MODE (gs_info->offset_vectype), is_load).exists (&vmode))
	len_p = true;
      ...
      if (len_p)
        {
          vec_loop_lens *lens = &LOOP_VINFO_LENS (loop_vinfo);
	        ...
	        vect_record_loop_len (loop_vinfo, lens, ncopies, vectype, factor);
	}
      else
        vect_record_loop_mask (loop_vinfo, masks, ncopies, vectype, scalar_mask);
      return;
  ...
  /* The loads and stores in RVV (RISC-V 'V' Extension) depend on both
     ...
     The current GCC can't make them both work,
     so we change the code here to work around
     and let len_load and len_store have higher priority. */
  machine_mode vmode;
	  ...
	  if (final_mask)
	    {
	      ...
	      if (loop_lens && direct_internal_fn_supported_p (IFN_LEN_MASK_STORE_LANES,
	        		tree_pair (TREE_TYPE (vec_array), vectype),
				OPTIMIZE_FOR_SPEED))
	        {
		  tree final_len = vect_get_loop_len (loop_vinfo, loop_lens, ncopies, j);
	  ...
	  else
	    {
	      ...
	      if (loop_lens && direct_internal_fn_supported_p (IFN_LEN_STORE_LANES,
	        		tree_pair (TREE_TYPE (vec_array), vectype),
				OPTIMIZE_FOR_SPEED))
	        {
		  tree final_len = vect_get_loop_len (loop_vinfo, loop_lens, ncopies, j);
		  call = gimple_build_call_internal (IFN_LEN_STORE_LANES, 2, vec_array, final_len);
		}
	      else
      ...
      else
	{
	  ...
	  for (i = 0; i < vec_num; i++)
	      ...
	      if (memory_access_type == VMAT_GATHER_SCATTER)
		{
		  ...
		  if (final_mask)
		    {
		      if (loop_lens && direct_internal_fn_supported_p (IFN_LEN_MASK_SCATTER_STORE,
				...
				OPTIMIZE_FOR_SPEED))
			{
			  ...
			  call = gimple_build_call_internal
		            (IFN_LEN_MASK_SCATTER_STORE, 6, dataref_ptr, vec_offset,
		             scale, vec_oprnd, final_mask, final_len);
			}
		      else
		       call = gimple_build_call_internal
		  ...
		  else if (loop_lens && direct_internal_fn_supported_p (IFN_LEN_SCATTER_STORE,
	        		tree_pair (vectype, TREE_TYPE (vec_offset)),
				OPTIMIZE_FOR_SPEED))
		    {
		      tree final_len
	      ...
	      if (final_mask)
		{
		  ...
		  gcall *call;
		  if (loop_lens && direct_internal_fn_supported_p (IFN_LEN_MASK_STORE,
	  ...
	  if (final_mask)
	    {
	      ...
	      unsigned int align = TYPE_ALIGN (TREE_TYPE (vectype));
	      tree alias_ptr = build_int_cst (ref_type, align);
	      if (loop_lens && direct_internal_fn_supported_p (IFN_LEN_MASK_LOAD_LANES,
	        		tree_pair (TREE_TYPE (vec_array), vectype),
				OPTIMIZE_FOR_SPEED))
	        {
		  tree final_len = vect_get_loop_len (loop_vinfo, loop_lens, ncopies, j);
	  ...
	  else
	    {
	      ...
	      data_ref = create_array_ref (aggr_type, dataref_ptr, ref_type);
	      if (loop_lens && direct_internal_fn_supported_p (IFN_LEN_LOAD_LANES,
	        		tree_pair (TREE_TYPE (vec_array), vectype),
				OPTIMIZE_FOR_SPEED))
	        {
		  tree final_len = vect_get_loop_len (loop_vinfo, loop_lens, ncopies, j);
		  call = gimple_build_call_internal (IFN_LEN_LOAD_LANES, 2, data_ref, final_len);
		}
	      else
		...
		case dr_unaligned_supported:
		  {
			...
			if (final_mask)
			  {
			    if (loop_lens && direct_internal_fn_supported_p (IFN_LEN_MASK_GATHER_LOAD,
	        		tree_pair (vectype, TREE_TYPE (vec_offset)),
				OPTIMIZE_FOR_SPEED))
			      {
				tree final_len
			...
			else if (loop_lens && direct_internal_fn_supported_p (IFN_LEN_GATHER_LOAD,
	        		tree_pair (vectype, TREE_TYPE (vec_offset)),
				OPTIMIZE_FOR_SPEED))
			  {
			    tree final_len
			    ...
			    call = gimple_build_call_internal
			      (IFN_LEN_GATHER_LOAD, 5, dataref_ptr,
			       vec_offset, scale, zero, final_len);
			  }
			else
			  call = gimple_build_call_internal
		    ...
		    if (final_mask)
		      {
			/* In RISC-V 'V' Extension, if we can brings the
			   length into maskload, we can save length configuration
			   instructions and register that hold length value. */
			tree ptr = build_int_cst (ref_type,
						  align * BITS_PER_UNIT);
			gcall *call;
			if (loop_lens && memory_access_type != VMAT_INVARIANT
			    && direct_internal_fn_supported_p (IFN_LEN_MASK_LOAD,
	        		tree_pair (vectype, TREE_TYPE (final_mask)),
				OPTIMIZE_FOR_SPEED))
			  {
			    tree final_len
			  	= vect_get_loop_len (loop_vinfo, loop_lens,
					             vec_num * ncopies,
					             vec_num * j + i);
			    call = gimple_build_call_internal (IFN_LEN_MASK_LOAD, 4,
							       dataref_ptr, ptr,
							       final_mask, final_len);
			  }
			else
...
/* Generate and return statement sequence that sets vector length LEN that is:
   ...
   ivtmp_10 = .WHILE_LEN (start_index, end_index, BITSIZE, NUNITS)
   ivtmp_11 = ivtmp_9 + ivtmp_10
...
void
vect_add_len_without_overflow (gimple_seq *header_seq, tree base, class loop *loop,
  	 tree ctrl, tree compare_type, tree ctrl_type, tree base_factor,
	   ...
	   tree *var_before, tree *var_after)
{
  ...
  tree initial;
  gimple_seq stmts;
  enum tree_code incr_op = MINUS_EXPR;
  edge pe = loop_preheader_edge (loop);
  tree va = make_temp_ssa_name (TREE_TYPE (base), NULL, "next_cnt");
  tree vb = make_temp_ssa_name (TREE_TYPE (base), NULL, "curr_cnt");
  tree step = make_temp_ssa_name (compare_type, NULL, "while_len");
  *var_before = vb;
  *var_after = va;

  tree var = gimple_build (header_seq, IFN_WHILE_LEN, compare_type,
	  		     			ctrl, build_int_cst (TREE_TYPE (base),
			       			GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (ctrl_type)))),
			       			base_factor);
  gimple* g = gimple_build_assign (step, var);
  gimple_seq_add_stmt (header_seq, g);

  stmt = gimple_build_assign (*var_after, incr_op, *var_before, step);
  ...
  initial = force_gimple_operand (base, &stmts, true, NULL_TREE);
  if (stmts)
    gsi_insert_seq_on_edge_immediate (pe, stmts);

  initial = force_gimple_operand (base, &stmts, true, NULL_TREE);
  phi = create_phi_node (*var_before, loop->header);
