...
static tree
vect_set_loop_controls_for_while_len (
    class loop *loop, loop_vec_info loop_vinfo, gimple_seq *preheader_seq,
    gimple_seq *header_seq, gimple_stmt_iterator loop_cond_gsi,
    rgroup_controls *rgc, tree niters, tree, bool)
{
  ...
  gimple_seq_add_stmt (preheader_seq,
                       gimple_build_assign (total_iters, nitems_total));
				 ...
				 tree niters_skip, bool might_wrap_p)
{
  ...
  if (!use_masks_p && !niters_skip && rgc->controls.length () == 1 &&
    ...
    return vect_set_loop_controls_for_while_len (
        loop, loop_vinfo, preheader_seq, header_seq, loop_cond_gsi, rgc, niters,
