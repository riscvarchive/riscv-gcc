...
riscv_legitimize_const_move (machine_mode mode, rtx dest, rtx src)
{
  ...
  if (GET_CODE (src) == CONST && GET_CODE (XEXP (src, 0)) == PLUS &&
      CONST_POLY_INT_P (XEXP (XEXP (src, 0), 1)))
    {
      ...
      riscv_vector_expand_poly_move (mode, reg, clobber, XEXP (XEXP (src, 0), 1));
                        ...
                        riscv_save_restore_fn fn)
{
  ...
  gcc_assert (SMALL_OPERAND (offset.to_constant ()));
  ...
  if (known_gt (size, 0))
    {
      ...
      if (known_gt (poly_offset, 0))
        {
          riscv_vector_adjust_frame (stack_pointer_rtx, poly_offset, false);
        }
    }
}
      ...
      if (!step1.is_constant ())
        {
          ...
          riscv_vector_adjust_frame (stack_pointer_rtx, poly_offset, true);
          step1 -= poly_offset;
        }
