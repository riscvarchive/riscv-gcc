...
extern GTY (()) tree
    acle_vector_types[MAX_TUPLE_SIZE][NUM_VECTOR_TYPES + 1][NUM_LMUL + 1];
...
inline rtx
expand_builtin_insn (enum insn_code icode, tree exp, rtx target,
                     vector_intrinsic_info info)
{
  gcc_assert (call_expr_nargs (exp) > 0);
  struct expand_operand ops[MAX_RECOG_OPERANDS];
  machine_mode mode = TYPE_MODE (TREE_TYPE (exp));
  tree fndecl = TREE_OPERAND (CALL_EXPR_FN (exp), 0);
  int pred = info.predicate;
  ...
  switch (info.predicate)
    ...
    case any_policy:
      {
        if (info.intrinsic_type != IT_none)
          {
            create_input_operand (&ops[opno++], const0_rtx, Pmode);
            if (info.intrinsic_type == IT_mask_tail ||
                info.intrinsic_type == IT_mask_only)
              create_input_operand (&ops[opno++], const0_rtx, Pmode);
          }
      }
      break;
    case ta_policy:
      {
        if (info.intrinsic_type == IT_tail_only_mask_first)
          add_input_operand (&ops[opno++], exp, offset++);
        create_input_operand (&ops[opno++], const0_rtx, Pmode);
        if (info.intrinsic_type == IT_mask_tail ||
            info.intrinsic_type == IT_mask_only ||
            info.intrinsic_type == IT_segment)
          create_input_operand (&ops[opno++], const0_rtx, Pmode);
      }
      break;
    case tu_policy:
      {
        if (info.intrinsic_type == IT_mask_tail ||
            info.intrinsic_type == IT_mask_tail_vd)
          create_input_operand (&ops[opno++], const0_rtx, Pmode);
        else if (info.intrinsic_type == IT_segment)
          {
            create_input_operand (&ops[opno++], const0_rtx, Pmode);
            create_input_operand (&ops[opno++], const0_rtx, Pmode);
          }
    ...
    case tama_policy:
      {
        add_input_operand (&ops[opno++], exp, offset++);
        if (info.intrinsic_type != IT_mask_tail_vd &&
            info.intrinsic_type != IT_mask_only_no_vd)
          create_input_operand (&ops[opno++], const0_rtx, Pmode);
      }
      break;
    default:
  ...
  if (info.intrinsic_type == IT_mask_only_no_vd)
    {
      pred = any_policy;
    }
  ...
  if (pred != -1)
    create_input_operand (&ops[opno++], GEN_INT (pred), Pmode);
...
/* Return the ACLE type vbool##LMUL##_t.  */
inline tree
lmul2mask_t (machine_mode mode, lmul_value_index lmul)
{
  ...
  index = ratio == 64   ? 0
          ...
          : ratio == 2  ? 5
                        : 6;
  return acle_vector_types[0][VECTOR_TYPE_bool][index];
}
...
/* Return the ACLE type for machine_mode && lmul_value_index && bool u.  */
inline tree
  ...
  if (VECTOR_MODE_P (mode))
    {
      ...
      if (GET_MODE_CLASS (mode) == MODE_VECTOR_BOOL)
        return acle_vector_types[0][VECTOR_TYPE_bool][lmul];

      switch (innermode)
        {
        case E_QImode:
          vectype = u ? acle_vector_types[0][VECTOR_TYPE_uint8][lmul]
                      : acle_vector_types[0][VECTOR_TYPE_int8][lmul];
          break;

        case E_HImode:
          vectype = u ? acle_vector_types[0][VECTOR_TYPE_uint16][lmul]
                      : acle_vector_types[0][VECTOR_TYPE_int16][lmul];
          break;

        case E_SImode:
          vectype = u ? acle_vector_types[0][VECTOR_TYPE_uint32][lmul]
                      : acle_vector_types[0][VECTOR_TYPE_int32][lmul];
          break;

        case E_DImode:
          vectype = u ? acle_vector_types[0][VECTOR_TYPE_uint64][lmul]
                      : acle_vector_types[0][VECTOR_TYPE_int64][lmul];
          break;

        case E_HFmode:
          vectype = acle_vector_types[0][VECTOR_TYPE_float16][lmul];
          break;

        case E_SFmode:
          vectype = acle_vector_types[0][VECTOR_TYPE_float32][lmul];
          break;

        case E_DFmode:
          vectype = acle_vector_types[0][VECTOR_TYPE_float64][lmul];
          break;
  ...
  if (VECTOR_MODE_P (mode))
    {
      ...
      switch (innermode)
        {
        case E_QImode:
          vectype = u ? acle_vector_types[nelt - 1][VECTOR_TYPE_uint8][lmul]
                      : acle_vector_types[nelt - 1][VECTOR_TYPE_int8][lmul];
          break;

        case E_HImode:
          vectype = u ? acle_vector_types[nelt - 1][VECTOR_TYPE_uint16][lmul]
                      : acle_vector_types[nelt - 1][VECTOR_TYPE_int16][lmul];
          break;

        case E_SImode:
          vectype = u ? acle_vector_types[nelt - 1][VECTOR_TYPE_uint32][lmul]
                      : acle_vector_types[nelt - 1][VECTOR_TYPE_int32][lmul];
          break;

        case E_DImode:
          vectype = u ? acle_vector_types[nelt - 1][VECTOR_TYPE_uint64][lmul]
                      : acle_vector_types[nelt - 1][VECTOR_TYPE_int64][lmul];
          break;

        case E_HFmode:
          vectype = acle_vector_types[nelt - 1][VECTOR_TYPE_float16][lmul];
          break;

        case E_SFmode:
          vectype = acle_vector_types[nelt - 1][VECTOR_TYPE_float32][lmul];
          break;

        case E_DFmode:
          vectype = acle_vector_types[nelt - 1][VECTOR_TYPE_float64][lmul];
          break;
                          ...
                          const char *extension)
{
  ...
  error_at (location, "ACLE function %qD requires ISA extension %qs", fndecl,
            extension);
  inform (location,
                                       ...
                                       vec<tree> &argument_types)
{
  ...
  if (*rfn_slot)
    printf ("intrinsic name1: %s, name2: %s\n", name,
            (*rfn_slot)->instance.get_func_name ());
  gcc_assert (!*rfn_slot);
  *rfn_slot = &rfn;
                                     ...
                                     predication_index pred) const
{
  ...
  if (pred == PRED_m || pred == PRED_tama || pred == PRED_tamu ||
      pred == PRED_tuma || pred == PRED_tumu)
    {
      if (GET_MODE_CLASS (TYPE_MODE (return_type)) == MODE_VECTOR_BOOL)
        argument_types.quick_insert (0, return_type);
      else
        argument_types.quick_insert (
            0, lmul2mask_t (GET_MODE_INNER (TYPE_MODE (return_type)), lmul));
    }

  /* check if dest parameter need. */
  if (pred == PRED_tu || pred == PRED_m || pred == PRED_tamu ||
      pred == PRED_tuma || pred == PRED_tumu)
    {
      unsigned int offset = pred == PRED_tu ? 0 : 1;
      if (argument_types.is_empty ())
        argument_types.quick_push (return_type);
      else
        argument_types.quick_insert (offset, return_type);
    }
...
function_builder::build_one (const function_instance &instance)
{
  ...
  apply_predication (
      ...
      argument_types, mode2lmul (mode), instance.get_pred ());
...
misc::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  return expand_builtin_insn (icode, exp, target, {IT_none, -1});
}
...
void
ternop::apply_predication (tree return_type, vec<tree> &argument_types,
                           lmul_value_index lmul, predication_index pred) const
{
  /* check if mask parameter need. */
  if (pred == PRED_m || pred == PRED_tama || pred == PRED_tamu ||
      pred == PRED_tuma || pred == PRED_tumu)
  ...
  /* check if dest parameter need. */
  unsigned int offset =
      (pred == PRED_m || pred == PRED_tama || pred == PRED_tamu ||
       pred == PRED_tuma || pred == PRED_tumu)
          ? 1
          : 0;
  if (argument_types.is_empty ())
...
void
reduceop::apply_predication (tree return_type, vec<tree> &argument_types,
                             lmul_value_index, predication_index pred) const
{
  /* check if mask parameter need. */
  if (pred == PRED_m || pred == PRED_tama || pred == PRED_tamu ||
      pred == PRED_tuma || pred == PRED_tumu)
    {
      argument_types.quick_insert (
          0, lmul2mask_t (GET_MODE_INNER (TYPE_MODE (argument_types[0])),
...
vle::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  return expand_builtin_insn (icode, exp, target,
                              {IT_mask_tail, instance.get_vma_vta ()});
}
...
void
vse::apply_predication (tree return_type, vec<tree> &argument_types,
                        lmul_value_index lmul, predication_index pred) const
{
  /* Apply a mask.  */
  if (pred == PRED_m)
...
vse::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  return expand_builtin_insn (icode, exp, target,
                              {IT_mask_only_no_vd, instance.get_vma_vta ()});
}
...
void
vsse::apply_predication (tree return_type, vec<tree> &argument_types,
                         lmul_value_index lmul, predication_index pred) const
{
  /* Apply a mask.  */
  if (pred == PRED_m)
...
{
  machine_mode mode = TYPE_MODE (TREE_TYPE (exp));
  return expand_builtin_insn (code_for_vlm (mode), exp, target,
                              {IT_none, instance.get_vma_vta ()});
}
...
{
  machine_mode mode = instance.get_arg_pattern ().arg_list[0];
  return expand_builtin_insn (code_for_vsm (mode), exp, target,
                              {IT_none, instance.get_vma_vta ()});
}
...
void
vsxei::apply_predication (tree return_type, vec<tree> &argument_types,
                          lmul_value_index lmul, predication_index pred) const
{
  /* Apply a mask.  */
  if (pred == PRED_m)
...
void
vlseg::apply_predication (tree return_type, vec<tree> &argument_types,
                          lmul_value_index lmul, predication_index pred) const
{
  /* Apply a mask.  */
  if (pred == PRED_m)
  ...
  if (instance.get_pred () == PRED_m)
    {
      ...
      unsigned int nelt =
          exact_div (GET_MODE_SIZE (tuple_mode), GET_MODE_SIZE (vector_mode))
              .to_constant ();
      argument_types.quick_push (get_tuple_t (vector_mode, unsigned_p, nelt));
    }
...
vlseg::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  return expand_builtin_insn (icode, exp, target,
                              {IT_segment, instance.get_vma_vta ()});
}
...
template <unsigned int NF>
void
vlseg_template<NF>::apply_predication (tree return_type,
                                       vec<tree> &argument_types,
                                       lmul_value_index lmul,
                                       predication_index pred) const
{
  ...
  if (pred == PRED_m)
    argument_types.quick_insert (
        NF, lmul2mask_t (GET_MODE_INNER (TYPE_MODE (return_type)), lmul));

  /* Apply a vmerge.  */
  if (pred == PRED_m)
    {
      for (unsigned int i = 0; i < NF; i++)
        argument_types.quick_insert (NF + 1 + i, return_type);
    }
                          ...
                          gimple_stmt_iterator *gsi_in, gcall *call_in) const
{
  ...
  if (instance.get_pred () == PRED_m)
    {
      for (unsigned int i = 0; i < NF; i++)
        {
          tree rhs_vector = gimple_call_arg (call_in, i + NF + 1);
  ...
  for (unsigned int i = NF; i < gimple_call_num_args (call_in); i++)
    {
      if (instance.get_pred () == PRED_m)
        {
          if (i == NF + 1)
            vargs.quick_push (tuple);
          else if (i > NF + 1 && i < 2 * NF + 1)
            continue;
          else
            vargs.quick_push (gimple_call_arg (call_in, i));
        }
  ...
  if (instance.get_pred () == PRED_void)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlseg%de%d_v_%s", NF,
    ...
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlseg%de%d_v_%s_tu", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else if (instance.get_pred () == PRED_m)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlseg%de%d_v_%s_m", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlseg%de%d_v_%s_mt", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  ...
  gassign *clobber = gimple_build_assign (tuple, build_clobber (tuple_type));
  gsi_insert_after (gsi_in, clobber, GSI_SAME_STMT);
...
template <unsigned int NF>
void
vlsegff_template<NF>::apply_predication (tree return_type,
                                         vec<tree> &argument_types,
                                         lmul_value_index lmul,
                                         predication_index pred) const
{
                            ...
                            gimple_stmt_iterator *gsi_in, gcall *call_in) const
{
  ...
  if (instance.get_pred () == PRED_void)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlseg%de%dff_v_%s", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else if (instance.get_pred () == PRED_tu)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlseg%de%dff_v_%s_tu", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else if (instance.get_pred () == PRED_m)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlseg%de%dff_v_%s_m", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlseg%de%dff_v_%s_mt", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
...
void
vsseg::apply_predication (tree return_type, vec<tree> &argument_types,
                          lmul_value_index lmul, predication_index pred) const
{
  /* Apply a mask.  */
  if (pred == PRED_m)
...
template <unsigned int NF>
void
vsseg_template<NF>::apply_predication (tree return_type,
                                       vec<tree> &argument_types,
                                       lmul_value_index lmul,
                                       predication_index pred) const
{
                          ...
                          gimple_stmt_iterator *gsi_in, gcall *call_in) const
{
  ...
  tree vector_type = get_dt_t (instance.get_arg_pattern ().arg_list[1],
                               unsigned_p, false, false);
  tree tuple_type = get_tuple_t (TYPE_MODE (vector_type), unsigned_p, NF);
  tree tuple = create_tmp_var (tuple_type, "tuple");
  auto_vec<tree, 16> vargs;
  ...
  if (instance.get_pred () == PRED_any)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vsseg%de%d_v_%s", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vsseg%de%d_v_%s_m", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
...
template <unsigned int NF>
void
vlsseg_template<NF>::apply_predication (tree return_type,
                                        vec<tree> &argument_types,
                                        lmul_value_index lmul,
                                        predication_index pred) const
{
                           ...
                           gimple_stmt_iterator *gsi_in, gcall *call_in) const
{
  ...
  if (instance.get_pred () == PRED_void)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlsseg%de%d_v_%s", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else if (instance.get_pred () == PRED_tu)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlsseg%de%d_v_%s_tu", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else if (instance.get_pred () == PRED_m)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlsseg%de%d_v_%s_m", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vlsseg%de%d_v_%s_mt", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
...
void
vssseg::apply_predication (tree return_type, vec<tree> &argument_types,
                           lmul_value_index lmul, predication_index pred) const
{
  /* Apply a mask.  */
  if (pred == PRED_m)
...
template <unsigned int NF>
void
vssseg_template<NF>::apply_predication (tree return_type,
                                        vec<tree> &argument_types,
                                        lmul_value_index lmul,
                                        predication_index pred) const
{
                           ...
                           gimple_stmt_iterator *gsi_in, gcall *call_in) const
{
  ...
  if (instance.get_pred () == PRED_any)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vssseg%de%d_v_%s", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vssseg%de%d_v_%s_m", NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (TYPE_MODE (vector_type))),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
...
void
vlxseg::apply_predication (tree return_type, vec<tree> &argument_types,
                           lmul_value_index lmul, predication_index pred) const
{
  /* Apply a mask.  */
  if (pred == PRED_m)
...
template <unsigned int NF, indexed_mode uo>
void
vlxseg_template<NF, uo>::apply_predication (tree return_type,
                                            vec<tree> &argument_types,
                                            lmul_value_index lmul,
                                            predication_index pred) const
{
...
vlxseg_template<NF, uo>::fold (const function_instance &instance,
                               ...
                               gcall *call_in) const
{
  bool unsigned_p = instance.get_data_type_list ()[1] == DT_uptr;
  tree vector_type = get_dt_t (instance.get_arg_pattern ().arg_list[1],
                               unsigned_p, false, false);
  tree tuple_type = get_tuple_t (TYPE_MODE (vector_type), unsigned_p, NF);
  tree tuple = create_tmp_var (tuple_type, "tuple");
  ...
  if (instance.get_pred () == PRED_void)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vl%sxseg%dei%d_v_%s",
        uo == INDEXED_u ? u : o, NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (indexed_mode)),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else if (instance.get_pred () == PRED_tu)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vl%sxseg%dei%d_v_%s_tu",
        uo == INDEXED_u ? u : o, NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (indexed_mode)),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else if (instance.get_pred () == PRED_m)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vl%sxseg%dei%d_v_%s_m",
        uo == INDEXED_u ? u : o, NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (indexed_mode)),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
  else
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vl%sxseg%dei%d_v_%s_mt",
        uo == INDEXED_u ? u : o, NF,
        GET_MODE_BITSIZE (GET_MODE_INNER (indexed_mode)),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
...
void
vsxseg::apply_predication (tree return_type, vec<tree> &argument_types,
                           lmul_value_index lmul, predication_index pred) const
{
  /* Apply a mask.  */
  if (pred == PRED_m)
...
template <unsigned int NF, indexed_mode uo>
void
vsxseg_template<NF, uo>::apply_predication (tree, vec<tree> &argument_types,
                                            lmul_value_index lmul,
                                            predication_index pred) const
{
  /* Apply a mask.  */
  if (pred == PRED_m)
    argument_types.quick_insert (
        0, lmul2mask_t (GET_MODE_INNER (TYPE_MODE (argument_types[3])), lmul));
                               ...
                               gcall *call_in) const
{
  ...
  if (instance.get_pred () == PRED_any)
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vs%sxseg%dei%d_v_%s",
        uo == INDEXED_u ? u : o, NF,
        GET_MODE_BITSIZE (
            GET_MODE_INNER (instance.get_arg_pattern ().arg_list[2])),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
    ...
    snprintf (
        fn_instance->get_func_name (), NAME_MAXLEN, "vs%sxseg%dei%d_v_%s_m",
        uo == INDEXED_u ? u : o, NF,
        GET_MODE_BITSIZE (
            GET_MODE_INNER (instance.get_arg_pattern ().arg_list[2])),
        mode2data_type_suffix (TYPE_MODE (tuple_type), unsigned_p, false));
...
vadc::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  return expand_builtin_insn (icode, exp, target,
                              {IT_tail_only, instance.get_vma_vta ()});
}
...
vcmp::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  return expand_builtin_insn (icode, exp, target,
                              {IT_mask_only, instance.get_vma_vta ()});
}
...
vimac::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  return expand_builtin_insn (icode, exp, target,
                              {IT_mask_tail_vd, instance.get_vma_vta ()});
}
                            ...
                            vec<tree> &argument_types) const
{
  for (unsigned int i = 1; i < instance.get_arg_pattern ().arg_len; i++)
    {
      if (instance.get_operation () == OP_vxm && i == 3)
        argument_types.quick_push (
            get_dt_t (GET_MODE_INNER (instance.get_arg_pattern ().arg_list[i]),
                      instance.get_data_type_list ()[i] == DT_unsigned));
      else
...
void
vmerge::apply_predication (tree return_type, vec<tree> &argument_types,
                           lmul_value_index, predication_index pred) const
{
  argument_types.quick_push (size_type_node);

  if (pred == PRED_tu)
    argument_types.quick_insert (1, return_type);
}
...
vmerge::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  return expand_builtin_insn (
      icode, exp, target, {IT_tail_only_mask_first, instance.get_vma_vta ()});
}
...
vmlogic::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  rtx_code code = strcmp (instance.get_base_name (), "vmand") == 0  ? AND
                  : strcmp (instance.get_base_name (), "vmor") == 0 ? IOR
                                                                    : XOR;
  return expand_builtin_insn (code_for_vm_mm (code, mode), exp, target,
                              {IT_none, instance.get_vma_vta ()});
}
...
vmnlogic::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  rtx_code code = strcmp (instance.get_base_name (), "vmnand") == 0  ? AND
                  : strcmp (instance.get_base_name (), "vmnor") == 0 ? IOR
                                                                     : XOR;
  return expand_builtin_insn (code_for_vmn_mm (code, mode), exp, target,
                              {IT_none, instance.get_vma_vta ()});
}
...
vmlogicn::expand (const function_instance &instance, tree exp, rtx target) const
{
  ...
  return expand_builtin_insn (code_for_vmnot_mm (code, mode), exp, target,
                              {IT_none, instance.get_vma_vta ()});
}
...
{
  machine_mode mode = TYPE_MODE (TREE_TYPE (exp));
  return expand_builtin_insn (code_for_vmmv_m (mode), exp, target,
                              {IT_none, instance.get_vma_vta ()});
}
...
{
  machine_mode mode = TYPE_MODE (TREE_TYPE (exp));
  return expand_builtin_insn (code_for_vmnot_m (mode), exp, target,
                              {IT_none, instance.get_vma_vta ()});
}
...
{
  machine_mode mode = TYPE_MODE (TREE_TYPE (exp));
  return expand_builtin_insn (code_for_vmclr_m (mode), exp, target,
                              {IT_none, instance.get_vma_vta ()});
}
...
{
  machine_mode mode = TYPE_MODE (TREE_TYPE (exp));
  return expand_builtin_insn (code_for_vmset_m (mode), exp, target,
                              {IT_none, instance.get_vma_vta ()});
}
...
void
vcpop::apply_predication (tree return_type, vec<tree> &argument_types,
                          lmul_value_index lmul, predication_index pred) const
{
  /* check if mask parameter need. */
  if (pred == PRED_m)
...
void
vfirst::apply_predication (tree return_type, vec<tree> &argument_types,
                           lmul_value_index lmul, predication_index pred) const
{
  /* check if mask parameter need. */
  if (pred == PRED_m)
...
{
  machine_mode mode = instance.get_arg_pattern ().arg_list[1];
  return expand_builtin_insn (code_for_vmv_x_s (mode), exp, target,
                              {IT_none, -1});
}
...
{
  machine_mode mode = instance.get_arg_pattern ().arg_list[1];
  return expand_builtin_insn (code_for_vfmv_f_s (mode), exp, target,
                              {IT_none, -1});
}
...
void
vcompress::apply_predication (tree return_type, vec<tree> &argument_types,
                              lmul_value_index, predication_index pred) const
{
  if (pred == PRED_tu)
    {
...
void
vfmerge::apply_predication (tree return_type, vec<tree> &argument_types,
                            lmul_value_index, predication_index pred) const
{
  argument_types.quick_push (size_type_node);
