...
static void
cleanup_insn_op (rtx_insn *insn, rtx clobber)
{
  if (!reload_completed)
    return;
...
dolocalprepass (const basic_block bb)
{
  ...
  FOR_BB_INSNS (bb, insn)
  {
    ...
    if (scalar_move_insn_p (insn))
      {
        gcc_assert (use_vtype_p (insn, &clobber) && use_vl_p (insn, &clobber));
