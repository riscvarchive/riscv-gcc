...
tree acle_vector_types[MAX_TUPLE_SIZE][NUM_VECTOR_TYPES + 1][NUM_LMUL + 1];
...
/* Record that TYPE is an ABI-defined VECTOR type that contains SEW and LMUL
   ...
   mangling of the type.  ACLE_NAME is the <riscv_vector.h> name of the type.
 */
static void
...
#include "riscv-vector-builtins.def"
  ...
  for (unsigned int i = 0; i < NUM_VECTOR_TYPES; i++)
	   ...
	   lmul++)
	{
	  ...
	  /* abi_name and acle_name follows vector type implementation in LLVM.
	     Take sew = 8, lmul = 1/8 for example,
	     abi_name = __rvv_int8mf8_t,
	     acle_name = vint8mf8_t.
...
register_vector_type (unsigned int type, unsigned int lmul)
{
  ...
  acle_vector_types[0][type][lmul] = vectype;
}
		     ...
		     unsigned int lmul)
{
  ...
  /* The contents of the type are opaque, so we can define them in any
	...
	struct svfooxN_t { svfoo_t __val[N]; };
     ...
     arguments.  */
  tree vector_type = acle_vector_types[0][type][lmul];
  ...
  acle_vector_types[num_vectors - 1][type][lmul] = tuple_type;
}
...
/* Return true if TYPE is a built-in RVV type defined by the ABI or ACLE.  */
bool
...
#undef DEF_RISCV_ARG_MODE_ATTR
  ...
  NUM_INSN_FUNC = 0;
#define DEF_RVV_FUNCTION(BASE_NAME, CLASS_NAME, ARG_PATTERN, PREDS, OP_TYPES) \
  NUM_INSN_FUNC++;
...
#define DEF_RVV_FUNCTION(BASE_NAME, CLASS_NAME, ARG_PATTERN, PREDS, OP_TYPES) \
  all_vector_functions[func_idx++] = new riscv_vector::CLASS_NAME (           \
      #BASE_NAME, get_vector_arg_all_patterns ARG_PATTERN, PREDS, OP_TYPES,   \
      (REQUIRED_EXTENSIONS));
#include "riscv-vector-builtins-functions.def"
#undef DEF_RVV_FUNCTION
}
