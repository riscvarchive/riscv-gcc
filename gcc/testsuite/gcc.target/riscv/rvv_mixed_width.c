...
void mixedwidth_mask(int8_t *a, int32_t * b, int32_t* c, size_t n) {
  ...
  for (; vl = vsetvl_8m1(n);) {
    mask = vsetlt_vs_int8m1(*(vint8m1_t *)a, 5);
    const_one = vsplat_s_int32m4(1);
    vec_c = vload_int32m4_mask(mask, const_one /*maskedoff*/, c);
    *(vint32m4_t *)b = vec_c;
    /* in Jim's test case,
     ...
     vint8m1_t const_five = vsplat_s_int8m1(5);
