...
void mixedwidth_mask(int8_t *a, int32_t * b, int32_t* c, size_t n) {
  size_t vl;
  rvv_bool8_t mask;
  rvv_int32m4_t const_one, vec_c;
  for (; vl = rvv_setvl_8m1(n);) {
    mask = rvv_slt_vs_int8m1(*(rvv_int8m1_t *)a, 5);
    const_one = rvv_splat_s_int32m4(1);
    vec_c = rvv_le_int32m4_mask(mask, const_one /*maskedoff*/, c);
    *(rvv_int32m4_t *)b = vec_c;
    /* in Jim's test case,
     ...
     rvv_int8m1_t const_five = rvv_splat_s_int8m1(5);
     *(rvv_int32m4_t *)b = (*(rvv_int8m1 *)a < const_five) ? *(rvv_int32m4_t*)c : const_one;
     */
    a += vl;
    b += vl;
    c += vl;
