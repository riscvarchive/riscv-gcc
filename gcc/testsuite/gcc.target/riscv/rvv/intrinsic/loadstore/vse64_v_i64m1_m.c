...
void
test0_vse64_v_i64m1_m(vbool64_t mask,int64_t *base,vint64m1_t value,size_t vl)
{
    vse64_v_i64m1_m(mask, base, value, vl);
}

/* { dg-final { scan-assembler-times {vsetvli\s+zero,\s*(?:ra|[sgtf]p|t[0-6]|s[0-9]|s10|s11|a[0-7]),\s*e64,\s*m1,\s*t[au],\s*mu} 1 } } */

void
test1_vse64_v_i64m1_m(vbool64_t mask,int64_t *base,vint64m1_t value,size_t vl)
{
    vse64_v_i64m1_m(mask, base, value, 23);
}
