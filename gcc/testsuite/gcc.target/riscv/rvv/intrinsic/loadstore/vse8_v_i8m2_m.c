...
void
test0_vse8_v_i8m2_m(vbool4_t mask,int8_t *base,vint8m2_t value,size_t vl)
{
    vse8_v_i8m2_m(mask, base, value, vl);
}

/* { dg-final { scan-assembler-times {vsetvli\s+zero,\s*(?:ra|[sgtf]p|t[0-6]|s[0-9]|s10|s11|a[0-7]),\s*e8,\s*m2,\s*t[au],\s*mu} 1 } } */

void
test1_vse8_v_i8m2_m(vbool4_t mask,int8_t *base,vint8m2_t value,size_t vl)
{
    vse8_v_i8m2_m(mask, base, value, 23);
}
