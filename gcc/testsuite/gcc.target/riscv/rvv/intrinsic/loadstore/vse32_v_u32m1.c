...
void
test0_vse32_v_u32m1(uint32_t *base,vuint32m1_t value,size_t vl)
{
    vse32_v_u32m1(base, value, vl);
}

/* { dg-final { scan-assembler-times {vsetvli\s+zero,\s*(?:ra|[sgtf]p|t[0-6]|s[0-9]|s10|s11|a[0-7]),\s*e32,\s*m1,\s*ta,\s*mu} 1 } } */

void
test1_vse32_v_u32m1(uint32_t *base,vuint32m1_t value,size_t vl)
{
    vse32_v_u32m1(base, value, 10);
}
