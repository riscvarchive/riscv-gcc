...
void
test0_vse16_v_u16m2(uint16_t *base,vuint16m2_t value,size_t vl)
{
    vse16_v_u16m2(base, value, vl);
}

/* { dg-final { scan-assembler-times {vsetvli\s+zero,\s*(?:ra|[sgtf]p|t[0-6]|s[0-9]|s10|s11|a[0-7]),\s*e16,\s*m2,\s*ta,\s*mu} 1 } } */

void
test1_vse16_v_u16m2(uint16_t *base,vuint16m2_t value,size_t vl)
{
    vse16_v_u16m2(base, value, 4);
}
