...
void
test0_vse16_v_i16m1_m(vbool16_t mask,int16_t *base,vint16m1_t value,size_t vl)
{
    vse16_v_i16m1_m(mask, base, value, vl);
}

/* { dg-final { scan-assembler-times {vsetvli\s+zero,\s*(?:ra|[sgtf]p|t[0-6]|s[0-9]|s10|s11|a[0-7]),\s*e16,\s*m1,\s*t[au],\s*mu} 1 } } */

void
test1_vse16_v_i16m1_m(vbool16_t mask,int16_t *base,vint16m1_t value,size_t vl)
{
    vse16_v_i16m1_m(mask, base, value, 4);
}
