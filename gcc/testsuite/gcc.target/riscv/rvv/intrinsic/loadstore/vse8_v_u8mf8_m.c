...
void
test0_vse8_v_u8mf8_m(vbool64_t mask,uint8_t *base,vuint8mf8_t value,size_t vl)
{
    vse8_v_u8mf8_m(mask, base, value, vl);
}

/* { dg-final { scan-assembler-times {vsetvli\s+zero,\s*(?:ra|[sgtf]p|t[0-6]|s[0-9]|s10|s11|a[0-7]),\s*e8,\s*mf8,\s*t[au],\s*mu} 1 } } */

void
test1_vse8_v_u8mf8_m(vbool64_t mask,uint8_t *base,vuint8mf8_t value,size_t vl)
{
    vse8_v_u8mf8_m(mask, base, value, 1);
}
