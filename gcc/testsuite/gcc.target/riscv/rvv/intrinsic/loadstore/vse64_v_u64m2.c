...
void
test0_vse64_v_u64m2(uint64_t *base,vuint64m2_t value,size_t vl)
{
    vse64_v_u64m2(base, value, vl);
}

/* { dg-final { scan-assembler-times {vsetvli\s+zero,\s*(?:ra|[sgtf]p|t[0-6]|s[0-9]|s10|s11|a[0-7]),\s*e64,\s*m2,\s*ta,\s*mu} 1 } } */

void
test1_vse64_v_u64m2(uint64_t *base,vuint64m2_t value,size_t vl)
{
    vse64_v_u64m2(base, value, 26);
}
