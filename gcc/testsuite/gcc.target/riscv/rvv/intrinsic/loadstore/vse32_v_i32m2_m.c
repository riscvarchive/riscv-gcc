/* { dg-do compile } */
/* { dg-skip-if "test vector intrinsic" { *-*-* } { "*" } { "-march=rv*v*" } } */
#include <stddef.h>
#include <riscv_vector.h>

void
test0_vse32_v_i32m2_m(vbool16_t mask,int32_t *base,vint32m2_t value,size_t vl)
{
    vse32_v_i32m2_m(mask, base, value, vl);
}

/* { dg-final { scan-assembler-times {vsetvli\s+zero,\s*(?:ra|[sgtf]p|t[0-6]|s[0-9]|s10|s11|a[0-7]),\s*e32,\s*m2,\s*t[au],\s*mu} 1 } } */

void
test1_vse32_v_i32m2_m(vbool16_t mask,int32_t *base,vint32m2_t value,size_t vl)
{
    vse32_v_i32m2_m(mask, base, value, 16);
}

/* { dg-final { scan-assembler-times {vsetivli\s+zero,\s*16,\s*e32,\s*m2,\s*t[au],\s*mu} 1 } } */

/* { dg-final { scan-assembler-times {vse32\.v} 2 } } */
