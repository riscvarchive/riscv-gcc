/* { dg-do compile } */
/* { dg-additional-options "-O2 -ftree-vectorize -mriscv-vector-bits=128 --save-temps" } */
/* { dg-skip-if "test vectorization using rvv" { *-*-* } { "*" } { "-march=rv*gc*v*zfh*" } } */
    
    

#include <stdint.h>

typedef int8_t vnx16i __attribute__((vector_size(16)));
typedef int16_t vnx8i __attribute__((vector_size(16)));
typedef int32_t vnx4i __attribute__((vector_size(16)));
typedef uint8_t vnx16ui __attribute__((vector_size(16)));
typedef uint16_t vnx8ui __attribute__((vector_size(16)));
typedef uint32_t vnx4ui __attribute__((vector_size(16)));
typedef __fp16 vnx8f __attribute__((vector_size(16)));
typedef float vnx4f __attribute__((vector_size(16)));

#define MASK_4 { 0,2,5,6 }
#define MASK_8 { 0,1,2,5,10,11,12,13 }
#define MASK_16 { 1,3,4,6,7,8,14,15,20,21,22,23,24,25,26,27 }

__rvv_int8m1_t  __attribute__((noinline, noclone)) test_1(__rvv_int8m1_t x,__rvv_int8m1_t y)
{
    return __builtin_shuffle ((vnx16i) x, (vnx16i) y, (vnx16i) MASK_16);
}


__rvv_int16m1_t  __attribute__((noinline, noclone)) test_2(__rvv_int16m1_t x,__rvv_int16m1_t y)
{
    return __builtin_shuffle ((vnx8i) x, (vnx8i) y, (vnx8i) MASK_8);
}


__rvv_int32m1_t  __attribute__((noinline, noclone)) test_3(__rvv_int32m1_t x,__rvv_int32m1_t y)
{
    return __builtin_shuffle ((vnx4i) x, (vnx4i) y, (vnx4i) MASK_4);
}


__rvv_uint8m1_t  __attribute__((noinline, noclone)) test_4(__rvv_uint8m1_t x,__rvv_uint8m1_t y)
{
    return __builtin_shuffle ((vnx16ui) x, (vnx16ui) y, (vnx16ui) MASK_16);
}


__rvv_uint16m1_t  __attribute__((noinline, noclone)) test_5(__rvv_uint16m1_t x,__rvv_uint16m1_t y)
{
    return __builtin_shuffle ((vnx8ui) x, (vnx8ui) y, (vnx8ui) MASK_8);
}


__rvv_uint32m1_t  __attribute__((noinline, noclone)) test_6(__rvv_uint32m1_t x,__rvv_uint32m1_t y)
{
    return __builtin_shuffle ((vnx4ui) x, (vnx4ui) y, (vnx4ui) MASK_4);
}


__rvv_float16m1_t  __attribute__((noinline, noclone)) test_7(__rvv_float16m1_t x,__rvv_float16m1_t y)
{
    return __builtin_shuffle ((vnx8f) x, (vnx8f) y, (vnx8i) MASK_8);
}


__rvv_float32m1_t  __attribute__((noinline, noclone)) test_8(__rvv_float32m1_t x,__rvv_float32m1_t y)
{
    return __builtin_shuffle ((vnx4f) x, (vnx4f) y, (vnx4i) MASK_4);
}


/* { dg-final { scan-assembler-times {\tvcompress.vm} 8 } } */

