...
int foo_rvv(float64_t *x, float64_t *y, int n) {
  ...
  vbool64_t mask;
  vec_t = vsplat_s_float64m1(0.0);
  vec_zero = vsplat_s_float64m1(0.0);
  for (;vl=vsetvl_64m1(n);) {
     vec_x = vload_float64m1(x);
     mask = vsetne_vs_float64m1(vec_x, 0.0);
  ...
  vec_sum = vmv_s_float64m1(vec_sum, 0.0); /* move scalar to vec_sum[0] */
