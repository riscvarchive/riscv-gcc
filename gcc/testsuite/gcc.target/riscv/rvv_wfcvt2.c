...
#define RVV_WFCVT_TEST(EM, WEM, MLEN, SEW, WSEW)			\
  ...
  {									\
    ...
    vx = vload_f##EM(x);						\
    vy = vload_i##WEM(y);						\
    vy = vcvt_i##WSEW##_f##SEW##_v_##EM##_mask (mask, vy, vx);	\
    vstore_i##WEM(y, vy);						\
  }									\
  ...
  {									\
    ...
    vx = vload_f##EM(x);						\
    vy = vload_u##WEM(y);						\
    vy = vcvt_u##WSEW##_f##SEW##_v_##EM##_mask (mask, vy, vx);	\
    vstore_u##WEM(y, vy);						\
  }									\
  ...
  {									\
    ...
    vx = vload_f##EM(x);						\
    vy = vload_i##WEM(y);						\
    vy = vcvt_rtz_i##WSEW##_f##SEW##_v_##EM##_mask (mask, vy, vx);	\
    vstore_i##WEM(y, vy);						\
  }									\
  ...
  {									\
    ...
    vx = vload_f##EM(x);						\
    vy = vload_u##WEM(y);						\
    vy = vcvt_rtz_u##WSEW##_f##SEW##_v_##EM##_mask (mask, vy, vx);	\
    vstore_u##WEM(y, vy);						\
  }									\
  ...
  {									\
    ...
    vx = vload_f##WEM(x);						\
    vy = vload_i##EM(y);						\
    vx = vcvt_f##WSEW##_i##SEW##_v_##EM##_mask (mask, vx, vy);	\
    vstore_f##WEM(x, vx);						\
  }									\
  ...
  {									\
    ...
    vx = vload_f##WEM(x);						\
    vy = vload_u##EM(y);						\
    vx = vcvt_f##WSEW##_u##SEW##_v_##EM##_mask (mask, vx, vy);	\
    vstore_f##WEM(x, vx);						\
  }									\
  ...
  {									\
    ...
    vx = vload_f##WEM(x);						\
    vy = vload_f##EM(y);						\
    vx = vcvt_f##WSEW##_f##SEW##_v_##EM##_mask (mask, vx, vy);	\
    vstore_f##WEM(x, vx);						\
  }
